...
(async () => {
    ...
    const a = 1;
    const b = 2;
    let response: any = await client.add(1, 2);
    print(`${a} + ${b} = ${response} as ${typeof response}`);

    const arr = [1, 2, 3];
    const index = 2;
    response = await client.getValueAtIndex(arr, index);
    print(`Value at index ${index} in [${arr.join(', ')}] = ${response} as ${typeof response}`);

    const obj = { a: 1, b: 2, c: 3 };
    const key = 'b';
    response = await client.getValueAtProperty(obj, key);
    print(`Value at key [${key}] in <pre>${JSON.stringify(obj, null, 2)}</pre> = ${response} as ${typeof response}`);

    response = [
        await client.getValueAtProperty(obj, key),
        await client.getValueAtIndex(arr, index),
    ];
    ...
    const key2 = 'c';
    response = await client.getValueAtProperty(obj2, key2);
